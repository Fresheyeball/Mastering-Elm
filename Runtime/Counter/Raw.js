...
function messageToString(value)
{
	...
	switch (keys.length)
	{
		...
		default:
			return value.ctor + ' â€¦ ' + messageToString(value[keys[keys.length - 1]]);
	}
}
	...
	function (maybeKey, maybeIsClosed, description) {
		var arrow = function () {
			...
			if (_p1.ctor === 'Nothing') {
				...
				if (_p1._0 === true) {
					return _elm_lang$virtual_dom$VirtualDom_Expando$makeArrow('â–¸');
				} else {
					return _elm_lang$virtual_dom$VirtualDom_Expando$makeArrow('â–¾');
				}
			}
		...
		if (_p3.ctor === '[]') {
			...
			if (_elm_lang$core$Native_Utils.cmp(nextLength, 18) > 0) {
				return {
					...
					_1: {
						ctor: '::',
						_0: _elm_lang$virtual_dom$VirtualDom_Helpers$text('â€¦}'),
						_1: {ctor: '[]'}
					}
				};
			} else {
				...
				return _elm_lang$virtual_dom$VirtualDom_Expando$viewTinyHelp(
					function () {
						...
						if (_p12.ctor === 'Nothing') {
							...
							return A2(_elm_lang$core$Basics_ops['++'], _p12._0, ' â€¦');
						}
					}());
			}
	}
...
var Elm = {};
Elm['TEA'] = Elm['TEA'] || {};
Elm['TEA']['Counter'] = Elm['TEA']['Counter'] || {};
if (typeof _fresheyeball$its_actually_on_gitlab$TEA_Counter$main !== 'undefined') {
    _fresheyeball$its_actually_on_gitlab$TEA_Counter$main(Elm['TEA']['Counter'], 'TEA.Counter', {"types":{"unions":{"TEA.Counter.Count":{"args":[],"tags":{"Count":[]}}},"aliases":{},"message":"TEA.Counter.Count"},"versions":{"elm":"0.18.0"}});
}
...
var runElmProgram = Elm.TEA.Counter.fullscreen;
