...
btnManager =
    describe "Btn Manager"
        ...
        , test "Rewindings" <|
            \() ->
                Expect.equal (List.map (View.btnMgr Model.Rewinding) btns)
                    [ View.Up, View.Up, View.Up, View.Down, View.Up, View.Up, View.Up ]
        , test "FastForwarding" <|
            \() ->
...
update =
    describe "Update"
        ...
        , describe "Insert"
            [ fuzz status "Insert works if Ejected, otherwise it does nothing" <|
                \randomStatus ->
                    if randomStatus == Model.Ejected then
                        statusEq Msg.Insert randomStatus Model.Stopped
                    else
                        statusEq Msg.Insert randomStatus randomStatus
            , fuzz msg "No messages have an effect if Ejected, except Insert" <|
                \randomMsg ->
                    if randomMsg == Msg.Insert then
                        statusEq randomMsg Model.Ejected Model.Stopped
                    else
                        statusEq randomMsg Model.Ejected Model.Ejected
            ]
        ]
